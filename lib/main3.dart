...
class _HomeScreenState extends State<HomeScreen3> {
    ...
    return Scaffold(
      ...
      body: FutureBuilder(
        ...
        builder: (context, AsyncSnapshot<List<dynamic>> snapshot) {
          ...
          final Map<String, List<FileSystemEntity>> categorized = snapshot.data![2];
  ...
  Widget _buildContent(StorageInfo info, List<FileSystemEntity> recents,
  ...
  Widget _buildStorageOverview(StorageInfo info) {
    final usedPercent = info.totalSpace > 0 ? info.usedSpace / info.totalSpace : 0;
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                const Text("Device Storage", style: TextStyle(fontWeight: FontWeight.bold)),
                Row(
                  children: [
                    Text("${_formatBytes(info.freeSpace)} Free", style: const TextStyle(color: Colors.grey)),
  ...
  Widget _buildCategories(Map<String, List<FileSystemEntity>> categorized) {
    final categories = [
      {'name': 'Photos', 'icon': Icons.photo, 'files': categorized['image'] ?? []},
      {'name': 'Videos', 'icon': Icons.video_collection, 'files': categorized['video'] ?? []},
      {'name': 'Documents', 'icon': Icons.description, 'files': categorized['document'] ?? []},
      {'name': 'Audio', 'icon': Icons.audiotrack, 'files': categorized['audio'] ?? []},
    ];
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final size = _calculateCategorySize(category['files'] as List<FileSystemEntity>);
              return CategoryCard(
                icon: category['icon'] as IconData,
                name: category['name']! as String,
                size: _formatBytes(size),
                count: (category['files'] as List).length,
                onTap: () => _openCategory(context, category['name']! as String, category['files'] as List<FileSystemEntity>),
              );
            },
          ),
        ),
  ...
  Widget _buildStorageDetails(StorageInfo info, Map<String, List<FileSystemEntity>> categorized) {
    final total = info.totalSpace;
    final categories = [
      {'name': 'Images', 'files': categorized['image'] ?? [], 'color': Colors.blue},
      {'name': 'Videos', 'files': categorized['video'] ?? [], 'color': Colors.green},
      {'name': 'Documents', 'files': categorized['document'] ?? [], 'color': Colors.orange},
      {'name': 'Audio', 'files': categorized['audio'] ?? [], 'color': Colors.purple},
      {'name': 'Other', 'files': categorized['other'] ?? [], 'color': Colors.red},
    ];

    // Calculate percentages
    final List<Map<String, dynamic>> storageCategories = categories.map((category) {
      final size = _calculateCategorySize(category['files'] as List<FileSystemEntity>);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
            const Text("Storage Details", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                ...
                Row(
                  ...
                  children: [
                    Text(_formatBytes(info.totalSpace)),
                    Text(
                      "${_formatBytes(info.freeSpace)} Free | ${_formatBytes(info.usedSpace)} Used",
                      style: const TextStyle(color: Colors.grey),
                    ),
                  ],
                ),
  ...
  Widget _buildStorageCategoryItem(String name, double percent, String size, Color color) {
    return Padding(
  ...
  BorderRadius _getBarRadius(Map<String, dynamic> category, List<Map<String, dynamic>> allCategories) {
  ...
  // Navigation methods
  void _openCategory(BuildContext context, String category, List<FileSystemEntity> files) {
    Navigator.push(
      context,
      MaterialPageRoute(
...
Future<StorageInfo> getStorageInfo() async {
  try {
    ...
    final total = await _getTotalSpace(directory.path);
    final free = await _getFreeSpace(directory.path);
    return StorageInfo(
      totalSpace: total,
      usedSpace: total - free,
      freeSpace: free,
    ...
    return StorageInfo(
      ...
      usedSpace: ((512 * 0.136) * 1024 * 1024 * 1024).toInt(), // ~69.8 GB
      freeSpace: ((512 * 0.864) * 1024 * 1024 * 1024).toInt(),
    );
  }
}
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text("$count files", style: const TextStyle(color: Colors.grey, fontSize: 12)),
              const SizedBox(height: 4),
    ...
    return GestureDetector(
      ...
      child: Card(
        ...
        child: ListTile(
          ...
          title: Text(fileName),
          subtitle: Text("${_formatBytes(size)} • ${dateFormat.format(modified)}"),
          trailing: IconButton(
